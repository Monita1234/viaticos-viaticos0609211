...
public class ConsultaEmpleados extends Conexion {
   ...
   //public ArrayList<Ingresos> cajamenor() throws SQLException{
    ...
    ///reporte general
    public boolean registrogeneralCaja_menor(Registrogeneral registrogeneral){
        PreparedStatement ps = null;
        String sql = "INSERT INTO registrogeneral( Fecha, Persona_Servicio, "
                + "Detalle, , Valor_cuenta, Valor_Total) "               
                + "VALUES(?,?,?,?)";
        try {
            ps = con.prepareStatement(sql);       
            ps.setString(4, registrogeneral.getFecha());
            ps.setString(6, registrogeneral.getDetalle());
           ps.setString(8, registrogeneral.getValorTotal());
            ps.setString(9, registrogeneral.getValorTotal());
            ps.execute();
            return true;
